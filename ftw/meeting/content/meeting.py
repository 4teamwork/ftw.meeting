...
from ftw.arbeitsraum.content.utilities import finalizeftwSchema
...
#from ftw.poodle.content.poodle import Poodle, PoodleSchema
...
finalizeftwSchema(MeetingSchema, folderish=True, moveDiscussion=False)

MeetingSchema.changeSchemataForField('effectiveDate', 'settings')
MeetingSchema.changeSchemataForField('expirationDate', 'settings')

#we do this after finalizeftwSchema, oherwise the location field will
#be invisible
...
class Meeting(folder.ATFolder, Poodle, CalendarSupportMixin):
    ...
    def getResponsibilityInfos(self, userids):
        ...
        factory = component.getUtility(IResponsibilityInfoGetter, name="ftw.meeting.responsibility.infos")
    ...
    def getMeetingForms(self):
        ...
        if self.portal_properties.get('ftw_meeting_properties'):
            for item in getattr(self.portal_properties.get('ftw_meeting_properties'), 'meeting_form'):
